      ...
      switch (option) {
        ...
        case '-level':
          ...
          logEnv.detailLevel =
            level !== void 0
              ? level
              : typescript_exports.server.LogLevel.normal;
          break;
        case '-traceToConsole':
...
`;
  ...
  if (useWatchGuard) {
    ...
    sys4.watchDirectory = (path, callback, recursive, options) => {
      ...
      if (status === void 0) {
        ...
        try {
          const args = [
            (0, typescript_exports.combinePaths)(libDirectory, 'watchGuard.js'),
            path,
          ];
          if (logger.hasLevel(typescript_exports.server.LogLevel.verbose)) {
            logger.info(
  ...
  function createLogger() {
    const cmdLineLogFileName =
      typescript_exports.server.findArgument('--logFile');
    const cmdLineVerbosity = getLogLevel(
      typescript_exports.server.findArgument('--logVerbosity')
    );
  ...
  function extractWatchDirectoryCacheKey(path, currentDriveKey) {
    ...
    if (isUNCPath(path)) {
      ...
      return firstSlash !== -1
        ? (0, typescript_exports.toFileNameLowerCase)(
...
function startNodeSession(options, logger, cancellationToken) {
  ...
  const _NodeTypingsInstallerAdapter = class _NodeTypingsInstallerAdapter extends typescript_exports
    ...
    createInstallerProcess() {
      if (
        this.logger.hasLevel(typescript_exports.server.LogLevel.requestTime)
      ) {
      ...
      this.installer = import_child_process.default.fork(
        typingsInstaller,
        args,
        { execArgv }
      );
  ...
  class IOSession extends typescript_exports.server.Session {
    ...
    event(body, eventName) {
      ...
      if (this.canUseEvents && this.eventPort) {
        if (!this.eventSocket) {
          if (
            this.logger.hasLevel(typescript_exports.server.LogLevel.verbose)
          ) {
            this.logger.info(
  ...
  class IpcIOSession extends IOSession {
    writeMessage(msg) {
      const verboseLogging = logger.hasLevel(
        typescript_exports.server.LogLevel.verbose
      );
      if (verboseLogging) {
  ...
  const disableAutomaticTypingAcquisition =
    typescript_exports.server.hasArgument(
  ...
  const commandLineTraceDir =
    typescript_exports.server.findArgument('--traceDirectory');
  ...
  function getGlobalTypingsCacheLocation() {
    switch (process.platform) {
      ...
      default:
        return typescript_exports.Debug.fail(
...
function start(
  ...
  if (
    ...
    /^development$/i.test(
      typescript_exports.sys.getEnvironmentVariable('NODE_ENV')
    )
  ) {
    typescript_exports.sys.tryEnableSourceMapsForHost();
  }
  ...
  console.warn = (...args2) =>
    logger.msg(
      args2.length === 1 ? args2[0] : args2.join(', '),
      typescript_exports.server.Msg.Err
    );
  console.error = (...args2) =>
    logger.msg(
      args2.length === 1 ? args2[0] : args2.join(', '),
  ...
  startServer(
    {
      ...
      allowLocalPluginLoads: typescript_exports.server.hasArgument(
        '--allowLocalPluginLoads'
      ),
      useSingleInferredProject: typescript_exports.server.hasArgument(
        '--useSingleInferredProject'
      ),
      useInferredProjectPerProjectRoot: typescript_exports.server.hasArgument(
        '--useInferredProjectPerProjectRoot'
      ),
      suppressDiagnosticEvents: typescript_exports.server.hasArgument(
        '--suppressDiagnosticEvents'
      ),
      noGetErrOnBackgroundUpdate: typescript_exports.server.hasArgument(
        '--noGetErrOnBackgroundUpdate'
      ),
      canUseWatchEvents: typescript_exports.server.hasArgument(
        '--canUseWatchEvents'
      ),
      serverMode,
    },
